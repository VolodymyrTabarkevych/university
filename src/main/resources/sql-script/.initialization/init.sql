CREATE TABLE subjects(id serial primary key, name VARCHAR(60) NOT NULL);
CREATE TABLE rooms(id serial primary key, number INTEGER);
CREATE TABLE teachers(id serial primary key, first_name VARCHAR(30) NOT NULL, last_name VARCHAR(30) NOT NULL);
CREATE TABLE teacherssubjects(teacher_id integer DEFAULT '0', subject_id integer DEFAULT '0', FOREIGN KEY (teacher_id) REFERENCES teachers(id) on delete cascade on update cascade, FOREIGN KEY (subject_id) REFERENCES subjects(id));
