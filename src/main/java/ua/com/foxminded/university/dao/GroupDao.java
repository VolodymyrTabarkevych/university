...
public class GroupDao implements CrudDao<Group> {
    ...
    private RowMapper<Group> groupRowMapper = (ResultSet resultSet, int i) -> {
        Group group = Group.builder().id(resultSet.getInt("id")).name(resultSet.getString("name")).build();
        group.setStudents(new HashSet<Student>(studentDaoJdbcTemplateImpl.findAllByGroupId(group.getId())));
        return group;
    };
